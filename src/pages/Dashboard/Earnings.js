  ...
  useEffect(() => {
    const fetchEarnings = async () => {
      try {
        ...
        const totalEarnings = orderSnapshot.docs.reduce((acc, doc) => acc + parseFloat(doc.data().price || 0), 0); // Ensure price is a number
        ...
        // In case of error, reset the earnings to a valid state
        setEarnings(0);
        setGST(0);
        setAppTax(0);
